package top.openyuan.jpa.core.repository.support;
...
/**
 * 继承了 {@link SimpleJpaRepository} 类，实现了 {@link JpaPlusRepository} 接口的 Fast JPA Repository 基础实现类.
 *
...
public class SimpleJpaPlusRepository<T, ID> extends SimpleJpaRepository<T, ID> implements JpaPlusRepository<T, ID> {
     ...
     */
    public SimpleJpaPlusRepository(JpaEntityInformation<T, ?> entityInformation, EntityManager entityManager) {
     ...
     */
    public SimpleJpaPlusRepository(Class<T> domainClass, EntityManager em) {
     ...
     *
     * <p>该方法会批量 {@code flush} 数据到数据库中，每次默认的批量大小为 {@link JpaConfigConstant#DEFAULT_BATCH_SIZE}.
    ...
    public <S extends T> void saveBatch(Iterable<S> entities) {
        this.saveBatch(entities, JpaConfigConstant.DEFAULT_BATCH_SIZE);
    }
    ...
    public <S extends T> void updateBatch(Iterable<S> entities) {
        this.updateBatch(entities, JpaConfigConstant.DEFAULT_BATCH_SIZE);
    }
    ...
    public void deleteBatchByIds(Iterable<ID> ids) {
        this.deleteBatchByIds(ids, JpaConfigConstant.DEFAULT_BATCH_SIZE);
    }
    ...
    public void deleteBatchByIds(Iterable<ID> ids, int batchSize) {
        ...
        String sql = StringUtils.format("delete from {} where {} in :batch_ids", entityName, idName);
